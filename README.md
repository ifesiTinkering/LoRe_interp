# LoRe basis interpretability — Rachel Freedman's APA code + extensions

## What I added on top of Rachel's code

This fork's contribution sits at the top so it's easy to find:

1. **`lore_basis_interpretability.ipynb`** — Colab notebook that runs all of the analyses below end-to-end. Loads Rachel's published `W_seen_K8.pt` and produces every figure in this README. Self-contained; uses only `torch`, `numpy`, `matplotlib`, `sklearn`, `scipy`.
2. **`plot_combined_heatmap.py`** — calls her `fig_user_weights_grid` to reproduce her exact Figure 2 (top panel), and stacks an extended bottom panel below it covering more users.
3. **`plot_user_similarity.py`** — pairwise dot-product heatmap across all 182 PRISM users from her checkpoint, with rows/cols reordered by hierarchical clustering on Euclidean distance.
...
The most actionable finding came from the basis-dominance plot in the notebook. Across Rachel's 182 published PRISM users:

- **All 8 bases are used.** Each is the dominant basis (largest `|w_k|`) for between 9% (V6) and 15% (V4) of users. No basis is dead; no basis is runaway-dominant. Rachel's K=8 is doing real work.
...
**Takeaway:** the per-row normalization in Rachel's published Figure 2 (each row scaled to its own max) makes weight differences look visually crushing, but the actual ratios are small. LoRe at K=8 on PRISM is best read as discovering a continuous mixture over 8 axes — not 8 discrete user archetypes.

## Future work

The natural next experiment: **does this basis-dominance distribution look different within demographic groups?** For example, do users who self-report caring most about "creativity" have a different dominance distribution than users who care most about "factuality"? If yes, that links Rachel's discovered basis to human-readable preference categories.

The blocker is that her published checkpoint doesn't ship a `user_id → row` mapping, so the 182 rows of `W_seen_K8.pt` are anonymous. Closing this gap requires re-running her training pipeline (`apa.load_prism` + `apa.train_lore_bases` at K=8) end-to-end on an A100 — roughly 6-8 hours for embedding generation plus ~30 minutes of training — and saving the in-memory ordering. Worth doing over a weekend; not committed to.

Once that mapping exists, the per-group dominance plots and a labeled version of the clustermap should follow easily.

## Repository contents

- **`apa/`** — Rachel Freedman's Adaptive Pluralistic Alignment code, distributed unmodified. Includes her pre-computed LoRe basis (`apa/experiments/checkpoints/V_K8.pt`), PRISM jury weights (`W_seen_K8.pt`), and simulated-jury weights (`W_adapted_hist_C016_C020_filtered.pt`).
...
Or open `lore_basis_interpretability.ipynb` in Colab for the full walkthrough including the dominance analysis. Everything uses her published checkpoints; nothing is retrained.
